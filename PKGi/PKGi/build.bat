...
for %%f in (*.cpp) do (
    clang++ -cc1 -Wno-logical-op-parentheses -triple x86_64-pc-freebsd-elf -munwind-tables -I"%OO_PS4_TOOLCHAIN%\\include" -I"%OO_PS4_TOOLCHAIN%\\include\\c++\\v1" -fuse-init-array -debug-info-kind=limited -debugger-tuning=gdb -emit-obj -o %intdir%\%%~nf.o %%~nf.cpp
)

for %%f in (..\..\_common\*.cpp) do (
    clang++ -cc1 -Wno-logical-op-parentheses -triple x86_64-pc-freebsd-elf -munwind-tables -I"%OO_PS4_TOOLCHAIN%\\include" -I"%OO_PS4_TOOLCHAIN%\\include\\c++\\v1" -fuse-init-array -debug-info-kind=limited -debugger-tuning=gdb -emit-obj -o %intdir%\%%~nf.o ..\..\_common\%%~nf.cpp
)
